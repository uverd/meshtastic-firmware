...
bool TreeShakeModule::handleReceivedProtobuf(const meshtastic_MeshPacket &mp, meshtastic_User *pptr)
{
    auto p = *pptr;
...
void TreeShakeModule::sendOurTreeShake(NodeNum dest, bool wantReplies, uint8_t channel, bool _shorterTimeout)
{
    ...
    monitorSensors();
    // if (prevPacketId) // if we wrap around to zero, we'll simply fail to
    // cancel in that rare case (no big deal)
...
TreeShakeModule::TreeShakeModule()
    : ProtobufModule("nodeinfo", meshtastic_PortNum_NODEINFO_APP, &meshtastic_User_msg), concurrency::OSThread("TreeShake")
{
